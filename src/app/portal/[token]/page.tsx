...
export default function PortalPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        setData(json);
        // Auto-expand the most recent month of pickups
        if (json.pickups?.length > 0) {
          const firstMonth = json.pickups[0].date.slice(0, 7);
  ...
  const pickupsByMonth: Record<string, PickupGroup[]> = {};
  for (const g of pickupGroups) {
    const key = g.date.slice(0, 7);
    if (!pickupsByMonth[key]) pickupsByMonth[key] = [];
    pickupsByMonth[key].push(g);
  }
  const monthKeys = Object.keys(pickupsByMonth).sort((a, b) => b.localeCompare(a));

  // Determine display: empresa is parent (or parentClient if this is a branch)
  const empresaName = parentClient ? parentClient.name : (branches.length > 0 ? client.name : null);
  const empresaRut = parentClient ? parentClient.rut : (branches.length > 0 ? client.rut : null);
  const sucursalName = parentClient ? client.name : null;
  const sucursalRut = parentClient ? client.rut : null;
  ...
  return (
    <div className="min-h-screen bg-[#f7f6f3]">
            ...
            <div className="flex flex-wrap items-start gap-3 mt-5">
              ...
              {empresaName ? (
                <div className="bg-white/10 backdrop-blur-sm rounded-xl px-4 py-2.5 border border-white/10">
                  <p className="text-white/60 text-xs flex items-center gap-1"><Building2 className="w-3 h-3" /> Empresa</p>
                  <p className="text-white font-semibold">{empresaName}</p>
                  {empresaRut && <p className="text-white/50 text-xs mt-0.5">RUT: {empresaRut}</p>}
                </div>
              ) : (
                <div className="bg-white/10 backdrop-blur-sm rounded-xl px-4 py-2.5 border border-white/10">
                  <p className="text-white/60 text-xs">Cliente</p>
                  <p className="text-white font-semibold">{client.name}</p>
                  {client.rut && <p className="text-white/50 text-xs mt-0.5">RUT: {client.rut}</p>}
                </div>
              )}

              {/* Sucursal (si este token es de una sucursal) */}
              {sucursalName && (
                <div className="bg-white/10 backdrop-blur-sm rounded-xl px-4 py-2.5 border border-white/10">
                  <p className="text-white/60 text-xs flex items-center gap-1"><Store className="w-3 h-3" /> Sucursal</p>
                  <p className="text-white font-semibold">{sucursalName}</p>
                  {sucursalRut && <p className="text-white/50 text-xs mt-0.5">RUT: {sucursalRut}</p>}
                </div>
              )}

              {/* Sucursales listadas (si es empresa madre) */}
              {branches.length > 0 && (
                <div className="bg-white/10 backdrop-blur-sm rounded-xl px-4 py-2.5 border border-white/10">
                  <p className="text-white/60 text-xs flex items-center gap-1"><Store className="w-3 h-3" /> {branches.length} {branches.length === 1 ? "Sucursal" : "Sucursales"}</p>
                  <div className="mt-1 space-y-0.5">
                    {branches.map((b) => (
                      <p key={b.id} className="text-white text-sm">{b.name}{b.rut ? <span className="text-white/40 ml-2 text-xs">RUT: {b.rut}</span> : ""}</p>
                    ))}
                  </div>
                </div>
              )}
      ...
      <div className="max-w-5xl mx-auto px-4 -mt-14 relative z-10 pb-12">
        ...
        {/* Certificates — collapsible, compact */}
        {certificates.length > 0 && (() => {
          const CERTS_PREVIEW = 3;
          const visibleCerts = showAllCerts ? certificates : certificates.slice(0, CERTS_PREVIEW);
          const hasMore = certificates.length > CERTS_PREVIEW;
          return (
            <div className="bg-white rounded-2xl shadow-sm border border-[#eae8e3] p-5 md:p-6 mb-6">
              <div className="flex items-center gap-2 mb-4">
                <div className="w-7 h-7 rounded-lg bg-[#e8f0e9] flex items-center justify-center">
        ...
        {/* Pickups — grouped by month, collapsible */}
        {pickupGroups.length > 0 && (
          <div className="bg-white rounded-2xl shadow-sm border border-[#eae8e3] p-5 md:p-6 mb-6">
            ...
            <div className="space-y-2">
              {monthKeys.map((mk) => {
                const groups = pickupsByMonth[mk];
                const isExpanded = expandedMonths.has(mk);
                const monthTotal = groups.reduce((s, g) => s + g.totalKg, 0);
                return (
                  <div key={mk} className="border border-[#eae8e3] rounded-xl overflow-hidden">
                    ...
                    <button
                      ...
                      className="w-full flex items-center justify-between px-4 py-3 bg-[#fafaf8] hover:bg-[#f5f3ef] transition-colors"
                    >
                      <div className="flex items-center gap-2">
                        <Calendar className="w-3.5 h-3.5 text-[#a3a3a3]" />
                        <span className="text-sm font-medium text-[#1a1a1a]">{monthLabel(mk)}</span>
                    ...
                    {/* Expanded pickup list */}
                    {isExpanded && (
                      <div className="border-t border-[#eae8e3] divide-y divide-[#eae8e3]">
                        {groups.map((group, i) => (
                          <div key={i} className="px-4 py-3">
                            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1 mb-2">
                              <span className="text-xs font-medium text-[#1a1a1a] capitalize">{formatFullDate(group.date)}</span>
                              <div className="flex items-center gap-2">
                                {group.location && (
                            ...
                            <div className="flex flex-wrap gap-1.5">
                              {group.materials.map((m) => (
                                <span
                                  key={m.material}
                                  className={`inline-flex items-center gap-1 text-[11px] font-medium px-2 py-0.5 rounded-md ${MATERIAL_BADGE_COLORS[m.material] || "bg-gray-50 text-gray-600"}`}
                                >
                                  <span className={`w-1.5 h-1.5 rounded-full ${MATERIAL_DOT_COLORS[m.material] || "bg-gray-400"}`} />
                                  {m.material} <span className="opacity-60">{m.kg.toLocaleString("es-CL")} kg</span>
                                </span>
                              ))}
                            </div>
                          </div>
